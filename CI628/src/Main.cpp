...
//Creates a connection to the serever client
...
//Runs when infomation is sent from CLIENT to SERVER
...
//The loop that runs containing the rendering events
void loop(SDL_Renderer* renderer) {
    ...
    while (is_running) {
        ...
        //Changes Background color and renders that
...
//Runs the base game and checks everything start properly
int run_game() {
    //Creates Window
    ...
    //Checks is window was made
    if (nullptr == window) {
        std::cout << "Failed to create window" << SDL_GetError() << std::endl;
        return -1;
    }

    //Creates the windows render
    ...
    //Runs the intialization Functions
    game->init(renderer);
    game->init_audio();
    game->initFont();
...
int main(int argc, char** argv) {
    ...
    // Checks if the IMG libary is loaded
    if ((IMG_Init(IMG_INIT_PNG) & IMG_INIT_PNG) != IMG_INIT_PNG) {
        printf("IMG_Init: %s\n", IMG_GetError());
        exit(3);
    }
    // Checks if the font libary is loaded
    if (TTF_Init() == -1) {
        printf("TTF_Init: %s\n", IMG_GetError());
        exit(3);
    }
